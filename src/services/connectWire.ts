...
const ENVELOPE_V1_SCHEMA = Uint8Array.from([
  0xf3, 0x50, 0x17, 0xc7, 0x74, 0x55, 0x8f, 0x19,
  0xf3, 0x50, 0x17, 0xc7, 0x74, 0x55, 0x8f, 0x19
]);
...
const encodeField = (payload: Uint8Array) => concatBytes(encodeU64(payload.length), payload);

const encodeStruct = (fields: Uint8Array[]) => concatBytes(...fields.map(encodeField));
...
const encodeByteVec = (bytes: Uint8Array) => concatBytes(encodeU64(bytes.length), bytes);

const encodeOption = <T>(value: T | null | undefined, encodeItem: (value: T) => Uint8Array) => {
...
const encodeByteArray = (bytes: Uint8Array) => concatBytes(...Array.from(bytes, (value) => encodeField(Uint8Array.of(value))));
...
const encodeDirection = (value: ConnectDirection) => encodeU32(value === 'wallet_to_app' ? 1 : 0);
...
const decodeDirectionPayload = (bytes: Uint8Array): ConnectDirection => {
  ...
  const value = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength).getUint32(0, true);
  if (value === 0) return 'app_to_wallet';
  if (value === 1) return 'wallet_to_app';
  throw new Error(`unsupported Connect direction tag: ${value}`);
};

const encodeRole = (value: 'app' | 'wallet') => encodeU32(value === 'wallet' ? 1 : 0);
...
const encodeAlgorithm = (value: number) => encodeU32(value);
...
const encodeAppMetaPayload = (appName: string, appUrl: string | null, iconHash: string | null) =>
  encodeStruct([
...
const encodeConstraintsPayload = (chainId: string) => encodeStruct([encodeString(chainId)]);
...
const encodeWalletSignaturePayload = (payload: {
  ...
  return encodeStruct([
    ...
    encodeVec(Array.from(signatureBytes), (value) => Uint8Array.of(value))
  ]);
};
  ...
  switch (payload.type) {
    case 'open': {
      const body = encodeStruct([
        encodeByteArray(hexToBytes(payload.appPublicKeyHex)),
        encodeOption(
          payload.appMeta,
    ...
    case 'approve': {
      const body = encodeStruct([
        encodeByteArray(hexToBytes(payload.walletPublicKeyHex)),
    ...
    case 'ping':
      return concatBytes(encodeU32(4), encodeU64(16), encodeField(encodeU64(payload.nonce)));
    case 'pong':
      return concatBytes(encodeU32(5), encodeU64(16), encodeField(encodeU64(payload.nonce)));
    default:
      throw new Error(`encoding for Connect control "${payload.type}" is not implemented`);
  }
};
...
export const encodeControlConnectFrame = (input: {
  ...
  return encodeStruct([
    encodeByteArray(base64UrlToBytes(input.sid)),
...
export const encodeOpenConnectFrame = (preview: ConnectPreview, input: {
  chainId: string;
  appName: string;
  appUrl: string | null;
  ...
  encodeControlConnectFrame({
    ...
    control: {
      ...
      signature: {
        ...
        algorithmLabel: input.signature.algorithmLabel || algorithmLabel(input.signature.algorithmCode),
...
export const encodePongConnectFrame = (sid: string, seq: number, nonce: number) =>
  encodeControlConnectFrame({
    sid,
    direction: 'app_to_wallet',
    seq,
...
const readBytes = (cursor: Cursor, length: number, label: string) => {
  ...
  if (!Number.isSafeInteger(length) || length < 0 || end > cursor.bytes.length) {
...
const readU64 = (cursor: Cursor, label: string) => {
  ...
  const value = new DataView(view.buffer, view.byteOffset, view.byteLength).getBigUint64(0, true);
...
const decodeBoolPayload = (bytes: Uint8Array, label: string) => {
  if (bytes.length !== 1) throw new Error(`${label} bool payload must be 1 byte`);
  return bytes[0] !== 0;
};

const decodeBytePayload = (bytes: Uint8Array, label: string) => {
  if (bytes.length !== 1) throw new Error(`${label} byte payload must be 1 byte`);
  return bytes[0];
};

const decodeFixedByteArrayPayload = (bytes: Uint8Array, itemCount: number, label: string) => {
  const cursor = createCursor(bytes);
  const out = new Uint8Array(itemCount);
  for (let index = 0; index < itemCount; index += 1) {
    const field = readField(cursor, `${label}[${index}]`);
    out[index] = decodeBytePayload(field, `${label}[${index}]`);
  }
  ensureExact(cursor, label);
  return out;
};

const decodeVecPayload = <T>(bytes: Uint8Array, label: string, decodeItem: (value: Uint8Array, label: string) => T) => {
  const cursor = createCursor(bytes);
...
const decodeOptionPayload = <T>(bytes: Uint8Array, label: string, decodeItem: (value: Uint8Array, label: string) => T) => {
  if (bytes.length === 0) throw new Error(`${label} option payload must not be empty`);
  if (bytes[0] === 0) {
    if (bytes.length !== 1) throw new Error(`${label} none option payload must be 1 byte`);
    return null;
  }
  if (bytes[0] !== 1) throw new Error(`${label} has unsupported option tag ${bytes[0]}`);
...
const decodePermissionsPayload = (bytes: Uint8Array, label: string): ConnectPermissions => {
  const cursor = createCursor(bytes);
  const methods = decodeVecPayload(readField(cursor, `${label}.methods`), `${label}.methods`, decodeStringPayload);
  const events = decodeVecPayload(readField(cursor, `${label}.events`), `${label}.events`, decodeStringPayload);
  const resources = decodeOptionPayload(
    readField(cursor, `${label}.resources`),
    `${label}.resources`,
    (payload, nestedLabel) => decodeVecPayload(payload, nestedLabel, decodeStringPayload)
  );
  ensureExact(cursor, label);
  return {
    methods,
...
const decodeSignInProofPayload = (bytes: Uint8Array, label: string): ConnectSignInProof => {
  const cursor = createCursor(bytes);
  const domain = decodeStringPayload(readField(cursor, `${label}.domain`), `${label}.domain`);
  const uri = decodeStringPayload(readField(cursor, `${label}.uri`), `${label}.uri`);
  const statement = decodeStringPayload(readField(cursor, `${label}.statement`), `${label}.statement`);
  const issuedAt = decodeStringPayload(readField(cursor, `${label}.issuedAt`), `${label}.issuedAt`);
  const nonce = decodeStringPayload(readField(cursor, `${label}.nonce`), `${label}.nonce`);
  ensureExact(cursor, label);
  return {
    domain,
    uri,
...
const decodeOpenControlBody = (bytes: Uint8Array): ConnectControlPayload => {
  const cursor = createCursor(bytes);
  const appPublicKey = decodeFixedByteArrayPayload(readField(cursor, 'open.app_pk'), 32, 'open.app_pk');
  const appMeta = decodeOptionPayload(readField(cursor, 'open.app_meta'), 'open.app_meta', (payload, label) => {
    const inner = createCursor(payload);
    const name = decodeStringPayload(readField(inner, `${label}.name`), `${label}.name`);
    const url = decodeOptionPayload(readField(inner, `${label}.url`), `${label}.url`, decodeStringPayload);
...
const decodeApproveControlBody = (bytes: Uint8Array): ConnectControlPayload => {
  const cursor = createCursor(bytes);
  const walletPublicKey = decodeFixedByteArrayPayload(readField(cursor, 'approve.wallet_pk'), 32, 'approve.wallet_pk');
  const accountId = decodeStringPayload(readField(cursor, 'approve.account_id'), 'approve.account_id');
  const permissions = decodeOptionPayload(
    readField(cursor, 'approve.permissions'),
    'approve.permissions',
    decodePermissionsPayload
  );
  const proof = decodeOptionPayload(readField(cursor, 'approve.proof'), 'approve.proof', decodeSignInProofPayload);
  const signature = decodeWalletSignaturePayload(readField(cursor, 'approve.sig_wallet'), 'approve.sig_wallet');
...
const decodeRejectControlBody = (bytes: Uint8Array): ConnectControlPayload => {
  const cursor = createCursor(bytes);
  const code = readU16(createCursor(readField(cursor, 'reject.code')), 'reject.code');
  const codeId = decodeStringPayload(readField(cursor, 'reject.code_id'), 'reject.code_id');
  const reason = decodeStringPayload(readField(cursor, 'reject.reason'), 'reject.reason');
...
const decodeCloseControlBody = (bytes: Uint8Array): ConnectControlPayload => {
  ...
  const code = readU16(createCursor(readField(cursor, 'close.code')), 'close.code');
  const reason = decodeStringPayload(readField(cursor, 'close.reason'), 'close.reason');
  const retryable = decodeBoolPayload(readField(cursor, 'close.retryable'), 'close.retryable');
...
const decodePingLikeControlBody = (bytes: Uint8Array, type: 'ping' | 'pong'): ConnectControlPayload => {
  const cursor = createCursor(bytes);
  const nonce = readU64(createCursor(readField(cursor, `${type}.nonce`)), `${type}.nonce`);
...
const decodeServerEventControlBody = (bytes: Uint8Array): ConnectControlPayload => {
  const cursor = createCursor(bytes);
  ...
  const eventBody = readBytes(eventCursor, eventBodyLength, 'server_event.body');
  ...
  const height = readU64(createCursor(readField(bodyCursor, 'server_event.height')), 'server_event.height');
  const entryHash = decodeStringPayload(readField(bodyCursor, 'server_event.entry_hash'), 'server_event.entry_hash');
...
const encodeEncryptedControlPayload = (payload: Extract<ConnectEnvelopePayload, { type: 'control_close' | 'control_reject' }>) => {
...
const encodeEnvelopePayload = (payload: ConnectEnvelopePayload) => {
  switch (payload.type) {
    ...
    default:
      throw new Error(`unsupported encrypted Connect payload: ${(payload as { type: string }).type}`);
  }
};

const decodeEncryptedControlPayload = (bytes: Uint8Array): Extract<ConnectEnvelopePayload, { type: 'control_close' | 'control_reject' }> => {
  const cursor = createCursor(bytes);
  const tag = readU32(cursor, 'encrypted_control.tag');
  switch (tag) {
    case 0: {
      const who = decodeRolePayload(readField(cursor, 'encrypted_control.close.who'));
      const code = readU16(
...
export const decodeConnectEnvelope = (bytes: Uint8Array): ConnectEnvelopeSummary => {
  if (bytes.length < 40) {
  ...
  if (flags !== 0) {
    throw new Error(`unsupported Norito flags for encrypted envelope: ${flags}`);
  }
  ...
  const cursor = createCursor(payload);
  const seq = readU64(createCursor(readField(cursor, 'envelope.seq')), 'envelope.seq');
  ...
  switch (tag) {
    ...
    case 1: {
      const domainTag = decodeStringPayload(
        readField(payloadCursor, 'envelope.payload.sign_request_raw.domain_tag'),
...
export const encodeConnectEnvelope = (seq: number, payload: ConnectEnvelopePayload) => {
...
const aadForCiphertextFrame = (sid: string, direction: ConnectDirection, seq: number) =>
  concatBytes(
    CONNECT_AAD_PREFIX,
...
export const encryptConnectEnvelope = (key: Uint8Array, sid: string, direction: ConnectDirection, seq: number, payload: ConnectEnvelopePayload) => {
  const envelope = encodeConnectEnvelope(seq, payload);
  const cipher = chacha20poly1305(key, nonceFromSeq(seq), aadForCiphertextFrame(sid, direction, seq));
  return cipher.encrypt(envelope);
};

export const decryptConnectEnvelope = (key: Uint8Array, sid: string, direction: ConnectDirection, seq: number, aead: Uint8Array) => {
  const cipher = chacha20poly1305(key, nonceFromSeq(seq), aadForCiphertextFrame(sid, direction, seq));
  const plaintext = cipher.decrypt(aead);
  const envelope = decodeConnectEnvelope(plaintext);
  if (envelope.seq !== seq) {
    throw new Error(`encrypted envelope sequence mismatch: frame=${seq} envelope=${envelope.seq}`);
  }
  return envelope;
};
...
export const decodeConnectFrame = (bytes: Uint8Array): ConnectFrameSummary => {
  const cursor = createCursor(bytes);
  const sidBytes = decodeFixedByteArrayPayload(readField(cursor, 'frame.sid'), 32, 'frame.sid');
  const direction = decodeDirectionPayload(readField(cursor, 'frame.dir'));
  const seq = readU64(createCursor(readField(cursor, 'frame.seq')), 'frame.seq');
  ...
  const cipherDirection = decodeDirectionPayload(readField(cipherCursor, 'ciphertext.dir'));
  const aead = decodeByteVecPayload(readField(cipherCursor, 'ciphertext.aead'), 'ciphertext.aead');
...
export const buildApprovePreimage = (input: {
  ...
  concatBytes(
    ...
    input.permissions ? hashConnectPermissions(input.permissions) : new Uint8Array(),
    input.proof ? hashConnectSignInProof(input.proof) : new Uint8Array()
  );

export const deriveConnectDirectionKeys = (preview: ConnectPreview, walletPublicKeyHex: string) => {
  const sharedSecret = x25519.getSharedSecret(
    hexToBytes(preview.privateKeyHex),
    hexToBytes(walletPublicKeyHex)
  );
  const salt = blake2b(concatBytes(SALT_PREFIX, base64UrlToBytes(preview.sid)), { dkLen: 32 });
  return {
